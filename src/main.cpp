...
void setup() {
  ...
  if (!dmx_driver_install(dmxPortA, &config, personalities, personality_count)) {
    Serial.println("[DMX] Port A driver install failed.");
  }
  if (!dmx_driver_install(dmxPortB, &config, personalities, personality_count)) {
    Serial.println("[DMX] Port B driver install failed.");
  }

  if (!dmx_set_pin(dmxPortA, transmitPinA, receivePinA, enablePinA)) {
    Serial.println("[DMX] Port A pin setup failed.");
  }
  if (!dmx_set_pin(dmxPortB, transmitPinB, receivePinB, enablePinB)) {
    Serial.println("[DMX] Port B pin setup failed.");
  }
